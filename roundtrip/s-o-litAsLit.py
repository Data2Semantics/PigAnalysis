...
rankedTriples = FOREACH rankedObjTriples {
		 
		
		newObjTuple = STRSPLIT(newObjrankedSubTriples::triples::obj, '@#@#', 2);
		newObj = (newObjTuple.$1 is null? newObjTuple.$0: newObjTuple.$1);
		
		GENERATE rankedSubTriples::triples::sub, rankedSubTriples::triples::pred, newObj, AVG({(rankedSubTriples::subRank is null? 0F: rankedSubTriples::subRank),(objRank is null? 0F: objRank)}) AS ranking;
